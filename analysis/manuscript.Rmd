...
Figure \ref{fig:sponsor_sector} shows the yearly distribution of central invoiced articles by year and institutional sector. While UK-based open access invoices were mainly addressed to discipline-specific governmental and non-profit research funders,  the situation is different in the Netherlands and Norway where national consortia of institutions belonging to the sector of higher education (e.g. universities) had transformative agreements with Elsevier. In 2019, Elsevier also launched similar agreements in other countries with lower publication output including Hungary and Poland. Besides, we found that invoiced funding bodies from the UK and the US mainly represent discipline-specific funders, while the remaining funding bodies from other countries are focused on a broad variety of disciplines (see Table \ref{tab:subject_profile}).
...
ind_sponsor_df %>%
  ...
  kbl("latex", booktabs = TRUE, escape = FALSE,
      ...
      align = c("lrrrrr"),
      linesep = "",
      caption = "Top 10 Elsevier hybrid OA sponsors by centrally invoiced articles 2015-2019.  The table furthermore presents the number of Elsevier hybrid journals with at least one invoiced article and two indicators of compliance: proportion of articles provided under a CC BY license and where spending was disclosed through the Open APC Initiative (OAPC). VSNU and Norway Institutes paid for OA through transformative agreements according to the ESAC Transformative Agreement Registry.") %>%
  row_spec(11:12, hline_after = TRUE) %>%
  add_header_above(c(" " = 4, "Compliance (in %)" = 2)) %>%
   kable_styling(latex_options = c("scale_down"))
```

The table also highlights the proportion of APC funding publicly disclosed through the Open APC initiative, suggesting that funders with a large CC BY share were also more willing to share their spending publicly. On the other hand, APC funding activities through transformative agreements were not publicly shared.

Figure \ref{fig:field_funder_matrix} shows the subject profile of OA funders relative to the journals’ subject fields. As expected, the figure shows that subject-specific funders tended to focus on a limited set of subject fields. In contrast, national consortia, such as VSNU and Norwegian Institutes (NO), financed OA articles  for a broader range of fields. However, within these consortia, the subject distribution also varied. VSNU OA spending, for example, had a strong focus on articles relating to Medicine, Environmental Science, and Energy, presumably reflecting disciplines with generally strong publication output at Dutch universities. Norwegian universities mostly published hybrid OA in Engineering, Environmental Science and Agricultural and Biological Sciences journals.  
