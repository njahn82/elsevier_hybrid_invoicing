...
knitr::knit_hooks$set(
  inline = function(x) {
    if (is.numeric(x)) {
      return(formatC(x, big.mark = ",", format = "g"))
    } else{
      return(x)
    }
  }
...
bind_cols(ind_summary_table, Total = `Total`) %>%
  rename(` ` = indicator) %>%
